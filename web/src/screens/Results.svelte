...
<div style="max-width:960px;margin:0 auto;padding:30px 24px 60px">
  ...
  {#if $state.running}
    <div style="background:#fff;border:1px solid #E7EBEF;border-radius:18px;box-shadow:0 1px 2px rgba(18,19,22,.05),0 8px 22px rgba(18,19,22,.06);padding:40px;text-align:center">
      ...
      <div style="margin-top:18px;font-weight:600;color:#1B1D21">Scraping search results…</div>
      <div style="margin-top:4px;font-size:.86rem;color:#8A93A0">{$state.progress.length ? $state.progress[$state.progress.length - 1] : 'Pulling videos, channel sizes and engagement for each link'}</div>
      <button on:click={cancel} disabled={$state.cancelling} style="margin-top:18px;background:#fff;border:1.5px solid #E7EBEF;border-radius:999px;padding:8px 16px;font-weight:600;font-size:.84rem;color:#1B1D21;cursor:pointer">{$state.cancelling ? 'Cancelling…' : 'Cancel run'}</button>
    </div>
