...
| **s3** | 1.3 | ch1 `s2.jpg` | steel glass of chai on a counter, india | ⚠ **this retires a standing blocker.** ch1 r1 blocked it for having no window; **style E's 1.3 names no window** — it names a counter and a steel glass, which is exactly the photograph |
...
| **s6** | 1.6 | ch1 `s5.jpg` | desk of paper documents | ⚠ **this closes the last open ch1 blocker.** The three named bills could not be bought (12 candidates, 2 queries), so they are **DRAWN** as three icon cells (§8). Ruling of 2026-08-07, carried forward intact |
...
| **s28** | 3.7 | ch1 `s7.jpg` | vintage weighing scale with brass weights | ⚠ **VERIFY BEFORE PROMOTING:** the carry-forward warns that 3.7's balance must put two clearly different weights in frame or it reads as a duplicate. Moving old `s7` here also **removes** the duplicate the carry-forward feared, because its own beat no longer exists |
