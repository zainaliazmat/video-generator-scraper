...
⚠ **CLOSED 2026-08-15 — the constraint this item existed for does not exist.** `vidiq_balance`
(free) reports **5,453 credits**: 5,380 renewable of a **6,000** cap resetting **2026-09-14**,
plus 73 add-on. The item said "only 13 remain until the 2026-08-29 reset", which was true on the
FREE tier (150/month) and is now stale in every part — the number, the cap and the reset date.
So R3 (title lock) + R3b (tag block) run in FULL at close-out, no rationing, no deferral; ~35
credits is 0.6% of the balance.

⚠ The stale figure had already cost something: it was about to buy a deliberately degraded
packaging pass on this video. **The lesson is the cheap one — `vidiq_balance` is free, so a
plan-level number must be re-read rather than carried forward from a run note.** The same stale
"150 renewable/month, 30 paid calls a month" claim heads `.claude/skills/vidiq/SKILL.md` §0,
which is the one home of that fact; corrected there in the same pass.
...
## `creator_frame_review_PASSED_2026-08-15`

**The creator reviewed the en batch artifacts and returned NO CHANGES**, so the §3b
"stop and wait" gate is discharged and all six chapters stand as locked. Artifacts
reviewed: `studio/videos/passive-income-number-en-FRAMES.png` (81 numbered cells) and
`…-en-PREVIEW.mp4` (7:48 stream-copy concat), both built 2026-08-12 05:42. Run resumed
2026-08-15 straight into assembly.

⚠ The hi lane is GONE (`finance-is-us-only`, 2026-08-15): this run's `chapters.hi.*`
entries, the seven-chapter hi column in `NEXT-SESSION-PROMPT.md` and every
`rulings_binding_on_both_cuts.*` heading are now history, not work. "Both cuts" in those
headings means en only. Nothing hi is owed and nothing hi should be resumed.

## `tool_fixes.cut_assemble_could_not_assemble_a_CHAPTER_FIRST_cut_2026-08-15`

⚠ **`tools/cut_assemble.py` could not assemble this cut at all, and the reason is
structural rather than a bug in the arithmetic.** It was written for
`japanese-money-methods`, where a full-length cut was built FIRST and chapters were
carved out of it for rework — so it read `studio/videos/<slug>-<cut>/index.html` for
scenes, audio rows, root duration, acts, root class and title, and rebuilt each chapter
from a `chapter.json` or an approved `index-claudedesign.html`.

**§3b now builds chapter-FIRST, which inverts that.** `passive-income-number-en` has no
full-length index.html — there was never anything to carve — and its chapters have
neither `chapter.json` nor `index-claudedesign.html`; each was generated by its own
`build.mjs`. So the assembler supported *neither* of its two chapter kinds on the
standing flow. Three failures, each of which had to be fixed at its source:

1. **`scaffold()` copied `package.json` and linked `node_modules` from the cut dir**,
   which a chapter-wise run never scaffolds — the chapters ARE the npm projects. Now
   resolved by looking (cut dir, then each chapter), with a real error naming the fix
   instead of a `FileNotFoundError` from inside `shutil.copy`.
2. **`assemble()` needed a shipped file for facts that were all still on disk.** New
   `from_chapters()`: `assets/voice/timing.json` is the one home of every start and
   duration (one VO line = one clip = one scene), and each chapter's own index.html
   holds its sections, audio rows and styling rebased to zero. **The chapter offset is
   the timing of its first line — the same constant `chapter_project.py` rebased BY —
   so this is that tool's exact inverse, not a re-derivation.** Verified before trusting:
   the six offsets chain 0 → 46.42 → 151.938 → 248.539 → 335.817 → 441.9 and land on
   527.873, which IS `timing.json`'s own total. `verify()` then checks the emitted DOM
   attribute-by-attribute against timing.json, so the merge is validated against the
   voice rather than against the chapters that produced it.
3. ⚠ **`assets/img` was the trap, and it is the one worth remembering.** `scaffold()`
   linked `fonts`/`img`/`voice`/`lottie` from the cut dir "if it exists". The cut's
   `assets/img/` DOES exist and holds a single `manifest.json` of the sourced
   photographs — **a different thing wearing the same name** as the design system's
   `img/` (grain.png, wm-en.png). Linking it shadowed both files into 404s, along with
   the fonts, gsap, lottie and the watermark. `fonts` and `img` are now linked from
   `tools/scaffold/assets` like the two stylesheets beside them, exactly as every
   chapter project already links them; only `voice`, `lottie` and gsap/lottie.min.js
   are resolved by search. **The general shape: a name test is not an identity test,
   and "it exists so it must be the right one" is how a design asset gets shadowed by
   an unrelated file with the same basename.**

**A fourth thing was found and deduped rather than merged.** Every chapter carries its
own copy of the RATE ASSERT IIFE, so a verbatim splice wrote six into one file, each
scanning all 81 scenes. They are not identical: **ch1's predates the
`derived_income_carries_assumption` extension and is missing the BILL branch.** ch1's is
a strict subset so it could not have thrown falsely here — but a stale assert in the
master would only ever surface during the 18-minute render, so `take_rate_assert()` now
keeps exactly ONE copy, the strictest, and prints how many variants it saw. Same rung as
`tool_fixes.three_upstream_bugs`: a second copy of a fix is how the bug comes back.

`hyperframes check` on the assembled master: **0 errors, 0 runtime errors, contrast
23/23 WCAG AA**, 4 lint warnings (file size, two dense tracks, and the 162-overlay count
ruled below).

## `owed.overlay_count_in_the_assembled_master` — RULED PROCEED, MEASURED 2026-08-15

**PROCEED. The 162 overlays were tested rather than argued about, and the failure does not
reproduce.** The linter confirms the note's arithmetic exactly — `composition_heavy_overlay_count_high`
counts **162** on the assembled master against a warn threshold of 25 and a field reproduction
at ~40 — so the exposure was real and the chapter renders did NOT retire it: each chapter only
ever had ~26–32 overlays in its render tree at once, and the field signal is explicitly
presence-based and duration-independent.

**THE PROBE, and it is the reusable part.** The failure signature is "solid black for the first
~half, recovering near the end", which is independent of duration — so it reproduces at ANY frame
rate. A full-length draft at **2 fps** renders the entire 81-scene overlay tree in 1,056 frames
instead of 15,836: **2m 15s, 1056/1056 captured**, versus 30–40 minutes for the real thing.

Measured on that file:
- `blackdetect` (d=0.1, pix_th=0.10): **zero black segments.**
- `signalstats` YAVG over all 1,056 frames: **min 36.97**, max 56.55, mean 46.51. Nothing
  remotely near black; the darkest frame in the cut is a normal graded ground.
- The signature itself, first half vs second half: **47.30 vs 45.72**. Flat. Per-tenth means run
  44.7–48.8 with no downward ramp anywhere.

⚠ **What the probe does NOT cover, stated so nobody quotes it as more than it is:** it ran at
2 fps, `-q draft`, `workerCount: 1`, `captureMode: beginframe`. The real render is 30 fps at
`-q high` with chunked encode and auto workers. What it DOES establish is that the full
162-overlay tree captures clean in a real Chrome capture on this box, which is the thing the
count was feared for. `check render`'s own black-segment scan remains the backstop on the actual
master, so a regression cannot ship silently.

**Generalises, and this is why it is written down:** a heavy-overlay count is cheap to TEST and
expensive to reason about. The note had been carrying this as an open decision since ch2 — six
chapters and a week — when 2m 15s of render would have settled it at any point. **When a warning
is about capture behaviour rather than content, render the whole thing at a low frame rate and
measure; do not argue from the count.**

## `owed.cue_density_is_UNHEARD_2026-08-15`

⚠ **RAISED AT ASSEMBLY, NOT A BLOCKER, AND NOBODY HAS EVER HEARD IT.** The merged cue list is
**128 cues** — `transition` ×80, `reveal` ×26, `hero` ×6, `chip` ×6, `stamp` ×5, `tick` ×3,
`buzz` ×1, `cta` ×1 — against `format.json tiers.medium.max_sfx_cues` of **30**. That is 4.3×,
and **80 transitions across 80 scene boundaries is one on literally every cut**, roughly one
sound every 6.6 seconds.

Two things make this an open question rather than a defect:

1. **Nothing enforces the budget in code.** `max_sfx_cues` appears only in the `tools/format/`
   snapshots read by `fin-audit` / `fin-evidence` / `fin-script` / `fin-storyboard`; neither
   `mix.py` nor `pipeline_check` reads it. So it is guidance to the writing stages that the
   cue GENERATOR never had to satisfy.
2. ⚠ **No chapter review heard any of it.** SFX and the music bed are laid by `tools/audio/mix.py`
   AFTER the render; a draft carries only the VO `<audio>` clips. So all six chapters were locked
   on sight and voice alone, and the 128 cues have never been played to anyone.

**Deliberately not changed before the render** — the video stream is unaffected by the mix, so
this costs nothing to defer and re-mixing is one command with no re-render. It belongs to the
proof-listen, and `format.json`'s own note is the argument to weigh there: *"a sound is
punctuation, and if every reveal has one then none of them means anything."*

## `owed.vo_drift_uses_one_global_onset_constant`

RAISED 2026-08-15 by the master QA, which failed `check render` on **VO drift +0.139s at line
4.8** against a ±0.1s target — the only line of 81 over, by **39 ms**.

**It was investigated to root cause rather than retried, and the finding is that assembly did
not cause it.** Three measurements: (1) all **81** audio rows sit on their `timing.json`
`audio_start` to within 1e-6 s, `vo-4-8` included; (2) 4.8's true speech onset measured on the
mp3 is **0.090 s** against a median of 0.050 and a max of 0.100 — top of the band, not outside
it; (3) ⚠ **the identical VAD path over ch4's own locked DRAFT measures +0.134 s on the same
line**, which the master reproduces to 5 ms.

⚠ **It surfaced now only because `check render` is a CUT-level gate** — `--chapter` is
explicitly not implemented for `render` — so it had never run against ch4. A chapter's gates are
`hyperframes check` and the draft render, and **neither measures VO drift**. So a per-clip timing
defect cannot be caught until the whole cut is assembled and rendered, which is the most
expensive possible moment to learn it. That is the real gap, not the 39 ms.

**The modelling defect:** drift is measured as `VAD_onset − vad_onset_latency_seconds(0.101) −
audio_start`, i.e. one GLOBAL constant stands in for every clip's onset. Real per-clip onset
variance on Brian spans **0.050–0.100 s** measured across all 81 clips, so a clip at the top of
the band eats 50 ms of a 100 ms budget before anything is wrong. **Drift measured against each
clip's OWN measured onset would separate "this clip starts late" (harmless) from "this clip is
placed wrong" (the thing the gate exists for).**

**Deliberately NOT changed mid-run**, same reasoning as `owed.cue_rung_5_does_two_jobs`: it
alters the verdict for every chapter of every video, and a cut in flight is the wrong place to
re-model a measurement. ⚠ And tuning `vo_drift_target_seconds` until this run passed was
rejected outright — bottom rung of `fix-defaults-not-gates`, and threshold-fitting to one's own
output is how a gate stops meaning anything.

Recorded as `rescued` (documented, terminal) rather than `done`, so run.json carries the reason
instead of a green light nobody can audit.

## `incidents.the_master_rendered_with_NO_MOTION_AT_ALL_2026-08-15`

⚠⚠ **THE FINDING OF THIS SESSION, and gate two is the only thing that caught it.** The first
assembled master rendered **81 static slides that merely cross-dissolved** — for 29 minutes,
566 MB — and passed every automated check on the way there.

**Cause, in `tools/cut_assemble.py:184` (`handbuilt`):**

```python
script = html[html.rindex("<script>") + len("<script>"):html.rindex("</script>")]
```

`rindex` takes the LAST inline `<script>`. That was correct for the chapters `handbuilt` was
written for — a hand-built `index-claudedesign.html` keeps its timeline in one block. **A
`build.mjs` chapter emits TWO**: the motion timeline (11,428 chars in ch2 — `sceneTransitions`,
`ken`, `rise`, `countUp`, `playLottie`, `artOp`) and then the RATE ASSERT (4,006 chars). The
merge took the assert and dropped the timeline. `take_rate_assert()` then correctly lifted the
assert out for dedup — leaving each chapter contributing an **empty body**.

**WHAT THIS COST ON SCREEN** (measured by `fin-review` against the chapter drafts at identical
timestamps): ch1 s2's chips all present at 4.10s instead of cascading at 5.2/5.8/6.3s; ch1 s6
fully ticked at 26.30s before anything should have ticked; ch2 s16 showing a static `95%` where
the draft is mid-count at `88%`; ch4 s46's tank already full instead of filling; ch6 s76's five
bars complete instead of three part-grown. Every cue landed at once, at t=0, on every scene.

⚠ **WHY NOTHING CAUGHT IT, and this is the reusable half.**

1. **`hyperframes check` reported `Motion ◇ 0 errors, 0 warnings`** — it validates the motion
   that IS there and has nothing to say about motion that is absent. It still reports 0 errors
   on the fixed file. **A checker that asks "is what ran well-formed" cannot see a thing that
   never ran.** Third instance of this exact shape on this run, after `stamp warn` red-on-red,
   `@commons` returning an unbounded original and `fade()` being a silent no-op — all four
   produced a WELL-FORMED WRONG RESULT rather than an error.
2. **The 2fps overlay probe could not see it either.** It measured luminance for black frames
   and reported a healthy flat profile — which a deck of static slides produces perfectly well.
   A probe answers only the question it was built for.
3. ⚠ **`FRAMES.png` and `PREVIEW.mp4` — the creator's sign-off artifacts — CANNOT show it, and
   did not.** `PREVIEW.mp4` is 528.085s, the exact sum of the six chapter drafts, written
   2026-08-12, **three days before `full/index.html` existed**. It is a stream-concat of the
   CHAPTER drafts, so it shows the correct motion from files that were never the master. **A
   still frame cannot show a missing ARRIVAL at all** — only that something is on screen, which
   it was. So the sign-off was real but never covered this artifact.

**FIXED, at the correct-default rung and then guarded:**

- `handbuilt` now walks **every** attribute-less `<script>` in document order instead of the
  last one, so both chapter topologies merge whole.
- ⚠ **New `verify_motion()`, and it is the part that matters.** It counts all 18 `motion.js`
  helpers in the merged master against the sum declared by the chapter source files and
  **refuses to write a master that carries fewer**. It fired correctly on the first re-run
  (`ken 77/78, pop 55/58, fade 52/54, draw 5/6, fill 6/7, span 18/21, plateKen 8/9`) — those
  were prose comments describing motion, in blocks the merge legitimately drops, so `calls()`
  now strips comments before counting and the comparison is code against code. The fixed
  master verifies **382 carried / 382 declared**, and re-check passes with `playLottie` 1,
  `countUp` 25, `plateKen` 9, `artOp` defined once and called twice, `--art-op:0.74` on
  s10/s57/s61.
- Where a carved cut's generated chapters take motion from the shipped cut's single script
  there is no per-chapter file to count, so it prints **"motion: not verified (no per-chapter
  source files)"** rather than a reassuring zero.

**THE LESSON, and it is about the gate and not the bug:** §3b says gate two's one job the
chapter passes cannot do is *sampling inside the cross-dissolves*. That was true and it also
undersold it. **Gate two is the only stage that ever watches the assembled master move.** Every
other check upstream reads either a chapter or a still. Cutting it — on the reasonable-sounding
grounds that "every scene has already been seen" — would have shipped this.

⚠ The boundaries themselves came back **clean** on the same review: all five joints sampled at
±0.30/0.10/0.225/0.38/0.60 plus 18 consecutive frames across ch4→ch5, no surviving outgoing
layer, no black, no freeze in 527.9s, watermark single and full-opacity at every midpoint, CSS
scoping intact (`.ch<N>` on all 13 chapter rules), and durations exact
(46.42+105.518+96.601+87.279+106.084+85.973 = root, with exactly the five chapter-final scenes
gaining +0.45). **So the assembler's timing arithmetic was right all along; it was the script
merge that was wrong** — which is why a timing-only `verify()` passed it.
