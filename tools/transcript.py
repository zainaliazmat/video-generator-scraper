...
def build(slug, cut):
    ...
    # The shipped composition, wherever this cut keeps it. A chapter-first run
    # (the standing §3b flow) never builds <slug>-<cut>/index.html — the master
    # is assembled into <slug>-<cut>-full/ instead, and IT is the file whose
    # `<audio data-start>` values are the ones actually rendered.
    comp = next((p for p in (
        ROOT / "studio" / "videos" / f"{slug}-{cut}" / "index.html",
        ROOT / "studio" / "videos" / f"{slug}-{cut}-full" / "index.html",
    ) if p.exists()), None)
    if not script.exists():
        sys.exit(f"missing: {script}")
    if comp is None:
        sys.exit(f"missing: no index.html in studio/videos/{slug}-{cut}/ "
                 f"or studio/videos/{slug}-{cut}-full/")
    ...
    head = (
        ...
        f"+ {comp.relative_to(ROOT)}. Never hand-edited — re-run the tool.\n"
        "---\n\n"
