...
OWED = ("proof-listen (hi, en) · thumbnail pick · caption upload (hi, en) · "
...
def milestone_note(slug, fmt):
    ...
    body = [
        ...
        "---",
        "",
        f"# {slug} — " + TODO.format("the two cuts' shipped titles, hi / en"),
        "",
...
def catalog_line(slug, fmt):
    ...
    state = "✅ RENDERED" if all(
        ...
        for c in (run.get("cuts") or ["hi", "en"])) else "🔧 IN PRODUCTION"
...
def promote_facts(slug, dry_run=False):
    ...
    # which market the number belongs to — and the ₹ and $ sets are explicitly not
    # convertible (every staging note opens with that firewall).
    rows, heading = [], None
    ...
    out = [f"\n{marker} ({date.today().isoformat()})\n",
           ...
           f"after both renders passed. SOFT/COMPUTED rows stay in staging by rule "
           f"(`fin-facts.md:32-36`). Section headings are kept because the ₹ and $ sets "
           f"are sourced independently and are not convertible.\n"]
    last = None
    ...
    try:
        ...
        pc.atomic_write_json(os.path.join(vault, "run.json"),
                             {"slug": "s", "topic": "T", "tier": "medium",
                              "architecture": "per-line-chapters", "cuts": ["hi", "en"],
                              "chapters": {"hi": {"1": {"status": "locked", "round": 2},
                                                  "2": {"status": "building"}},
                                           "en": {"1": {"status": "locked", "round": 1}}}})
        open(os.path.join(vault, "script-hi.md"), "w").write("**1.1**\n> एक\n")
        ...
        open(os.path.join(vault, "facts-staging.md"), "w").write(
            "# PART A\n"
            "| SIP minimum | ₹500 | AMFI | HARD |\n"
            "| Median salary | ₹29,000 | blog | SOFT |\n"
            "## India (₹)\n"
            "| Savings rate | 7.0% | RBI | HARD |\n")
        ...
        assert "2 of 3 chapters locked" in note, note
        # per CUT, never pooled: one cut can ship while the other is still building
        assert "| chapters locked | 1 | 1 |" in note, note
        assert "| review rounds | 2 | 1 |" in note, note
        ...
        pool = open(MONEY_FACTS, encoding="utf-8").read()
        assert "SIP minimum" in pool and "Savings rate" in pool
        assert "Median salary" not in pool, "a SOFT row was promoted"
        assert "**PART A**" in pool and "**India (₹)**" in pool, \
