#!/usr/bin/env bash
#
# ytauto - one-command runner for the terminal UI
# -----------------------------------------------
# Sets everything up the first time, then launches the TUI.
#
#   ./run.sh              # launch the terminal UI - pick a task, hit Enter
...
# Python code lives in backend/; data files (outputs, history/) stay here at root.
...
# 4. Launch the terminal UI. Make sure its deps exist even in a venv created
#    before a dependency was added (run.sh only pip-installs at venv creation).
